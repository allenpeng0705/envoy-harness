/**
 * OpenAI live tests (T3.6).
 *
 * **What this file tests:** the real network round-trip
 * for `OpenAIAdapter` via `createProviderAdapter`. The
 * hermetic suite (`test/llm-openai.test.ts`) covers the
 * wire shape via `FakeHttpClient`; this file covers
 * "the request actually returns a valid response from
 * `api.openai.com`" — the wiring that hermetic tests
 * can't see.
 *
 * **Opt-in:** runs only when BOTH `RUN_LIVE_TESTS=1` AND
 * `OPENAI_API_KEY` are set. See `./helpers.ts` for the
 ...
 * ~5 output tokens). OpenAI charges per token; this is
 * pennies per run. Don't loop it.
 */
...
liveDescribe("OpenAI live — simple completion", "OPENAI_API_KEY", () => {
  it(
    "completes a one-line prompt and reports usage",
    async () => {
      const adapter = createProviderAdapter({ provider: "openai" });
